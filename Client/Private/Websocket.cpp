...
void UWebSocket::SendMessage(uint8 PacketType, UByteBuffer* Message)
{
	...
	NewData.Reserve(2 + OriginalData.Num());
	NewData.Add(0);
...
{
	if (InternalWebSocket && InternalWebSocket->IsConnected() && !Key.IsEmpty())
	{
		const TArray<uint8>& OriginalData = Message->GetBuffer();
		TArray<uint8> NewData;
		...
		if (EncryptedData.Num() > 0) {
			...
			FinalData.Reserve(1 + EncryptedData.Num());
			FinalData.Add(1); 
			FinalData.Append(EncryptedData);
			InternalWebSocket->Send(FinalData.GetData(), FinalData.Num(), true);
		}
		else {
			UE_LOG(LogTemp, Error, TEXT("Error to encrypt"));
		}	
	}
}
